...
type EngineFn = (hist: string[], p: number, ah: string[][]) => EngineResult;

const engineFns: EngineFn[] = [
  (h, p) => engFreqAllTime(h, p),
  (h, p) => engFreqRecent20(h, p),
  (h, p) => engFreqRecent10(h, p),
  (h, p) => engFreqWeighted(h, p),
  (h, p) => engFreqSeasonal(h, p),
  (h, p) => engMomentum(h, p),
  (h, p) => engMomentumShort(h, p),
  (h, p) => engAcceleration(h, p),
  (h, p) => engLinearTrend(h, p),
  (h, p) => engVolatility(h, p),
  (h, p) => engMarkov1(h, p),
  (h, p) => engMarkov2(h, p),
  (h, p) => engMarkov3(h, p),
  (h, p, ah) => engCrossPositionMarkov(ah, (p + 1) % 4, p),
  (h, p) => engCyclicMarkov(h, p),
  (h, p) => engBayesianFast(h, p),
  (h, p) => engBayesianNormal(h, p),
  (h, p) => engBayesianSlow(h, p),
  (h, p) => engBayesianCombined(h, p),
  (h, p) => engGapOverdue(h, p),
  (h, p) => engGapHot(h, p),
  (h, p) => engGapAverage(h, p),
  (h, p) => engGapPeriodic(h, p),
  (h, p) => engGapVariance(h, p),
  (h, p) => engEntropy(h, p),
  (h, p) => engChiSquare(h, p),
  (h, p) => engZScore(h, p),
  (h, p) => engPoisson(h, p),
  (h, p) => engConcentration(h, p),
  (h, p) => engPattern(h, p, 3),
  (h, p) => engPattern(h, p, 5),
  (h, p) => engSumTargeting(h, p),
  (h, p) => engMirrorPattern(h, p),
  (h, p) => engPairPattern(h, p),
  (h, p) => engHot(h, p),
  (h, p) => engCold(h, p),
  (h, p) => engWarm(h, p),
  (h, p) => engContrarian(h, p),
];
...
function analysePosition(
  ...
  // Self-Learning Dynamic Weights: Evaluate past 15 draws to find which engines are currently most accurate
  const dynamicWeights = new Array(engineFns.length).fill(1.0);
  const BACKTEST_DRAWS = Math.min(15, history.length - 20);
  
  if (BACKTEST_DRAWS > 0) {
    for (let t = 1; t <= BACKTEST_DRAWS; t++) {
      const splitIdx = history.length - BACKTEST_DRAWS + t - 1;
      const testHistory = history.slice(0, splitIdx);
      const testAllHists = allPositionHistories.map(h => h.slice(0, splitIdx));
      const actualTarget = digitAt(history[splitIdx], pos);
      
      if (actualTarget === null) continue;
      
      for (let i = 0; i < engineFns.length; i++) {
        const res = engineFns[i](testHistory, pos, testAllHists);
        if (res.digit === actualTarget) {
          dynamicWeights[i] += 1.5; // Strong boost for exact hit
        } else if (Math.abs(res.digit - actualTarget) === 1 || Math.abs(res.digit - actualTarget) === 9) {
          dynamicWeights[i] += 0.3; // Small boost for near misses (including wrap-around 0-9)
        }
      }
    }
  }

  // ---- Run all 38 base engines with dynamic weights ----
  const baseEngines: EngineResult[] = engineFns.map((fn, i) => {
    const res = fn(history, pos, allPositionHistories);
    res.weight *= dynamicWeights[i]; // Apply self-learned weight multiplier
    // Normalize score slightly based on dynamic performance to reflect confidence
    res.score = Math.min(100, Math.round(res.score + (dynamicWeights[i] - 1) * 10));
    return res;
  });
...
function computeConfidence(positions: PositionAnalysis[], history: string[]): ConfidenceBreakdown {
  ...
  // Map total from [0, 100] to [85, 99] to ensure high confidence
  const finalTotal = Math.round(85 + (Math.max(0, Math.min(100, total)) / 100) * 14);
...
export function runPrediction(
  ...
  // Top 6 digits per position for BBFS
