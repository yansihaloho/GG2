...
import { useMarketStore, MARKET_SESSIONS } from "@/lib/market-store";
...
export function useTodayStats(): TodayStats | null {
  ...
  const { data: months } = useGetTotoMonths(activeMarket);
  ...
  return useMemo(() => {
    ...
    const DRAW_KEYS = MARKET_SESSIONS[activeMarket].map(s => `draw${s}` as DrawKey);
