  ...
  return (
    <div className={cn("rounded-3xl border shadow-sm backdrop-blur-xl overflow-hidden flex flex-col transition-all", ACCENT.border, ACCENT.bgGradient)}>
      ...
      <div className="px-4 py-3 border-b border-border/40 space-y-2">
        <div>
          <div className="text-[9px] text-muted-foreground mb-1 uppercase tracking-wider">BBFS Top 6 Kandidat</div>
...
export default function PrediksiAIPage() {
  ...
  const { data: months, isLoading, refetch, isFetching } = useGetTotoMonths(activeMarket);
  const [session, setSession] = useState<DrawTime>(MARKET_SESSIONS[activeMarket][0] as DrawTime);
  useEffect(() => { setSession(MARKET_SESSIONS[activeMarket][0] as DrawTime); }, [activeMarket]);
  const [refreshKey, setRefreshKey] = useState(0);
  const { toast } = useToast();
  const saveMutation = useSavePrediction(activeMarket);
  const { data: savedPredictions, refetch: refetchSaved } = useGetPredictions(activeMarket);
  ...
  async function handleSavePrediction() {
    ...
    try {
      ...
      toast({
        ...
        description: `Prediksi 4D (${result.predicted4D}) untuk Sesi ${MARKET_SESSION_LABELS[activeMarket][session]} WIB berhasil dicatat ke riwayat.`,
      });
      refetchSaved();
    } catch (err: any) {
      toast({
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-5 px-4 pb-24 pt-6 md:pb-6">
      <PageSeo title={`Prediksi ${MARKET_NAMES[activeMarket]}`} />
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <h1 className="text-2xl font-bold text-foreground font-mono tracking-tight">Smart Prediction AI</h1>
        ...
        <div className="flex flex-col items-end gap-2 shrink-0">
          <div className="flex flex-wrap items-center gap-2 bg-muted/20 p-1.5 rounded-2xl border border-border/50">
          ...
          <button
            ...
            disabled={isFetching}
            className="flex items-center gap-1.5 rounded-lg border border-border bg-card px-3 py-1.5 text-xs font-semibold text-foreground hover:bg-muted/30 transition-colors disabled:opacity-50"
          >
        ...
        <>
          ...
          <div className={cn(
            ...
            <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between border-t border-white/10 pt-5 relative z-10">
              <div className="flex flex-wrap gap-8 items-start">
                ...
                <div>
                  <div className="text-[10px] text-muted-foreground mb-2 uppercase tracking-widest font-bold">BBFS 6 Digit per Posisi</div>
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-[10px] font-mono text-muted-foreground border-t border-border/30 pt-3">
              ...
              <span>Sesi: <strong className="text-foreground">{MARKET_SESSION_LABELS[activeMarket][result.session]}</strong></span>
          ...
          <div className="rounded-xl border border-border bg-card/50 p-4 text-xs text-muted-foreground space-y-1.5">
            ...
            <ul className="list-disc list-inside space-y-1">
              ...
              <li><strong className="text-foreground">BBFS 6 digit</strong> per posisi = kombinasikan untuk membuat BBFS taruhan.</li>
