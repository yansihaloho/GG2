...
import type { MarketType } from "@/lib/market-store";
...
function SlotCard({ analysis, activeMarket, defaultOpen }: { analysis: SlotAnalysis; activeMarket: MarketType; defaultOpen: boolean; key?: any }) {
      ...
      >
        <div className="flex items-center gap-3">
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-primary/10 border border-primary/20">
            <span className="font-mono text-sm font-black text-primary">{MARKET_SESSION_LABELS[activeMarket][analysis.slot]}</span>
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="font-bold text-foreground">Draw {MARKET_SESSION_LABELS[activeMarket][analysis.slot]} WIB</span>
              {todayVal && (
...
function TrendChart({ analyses, activeMarket }: { analyses: SlotAnalysis[], activeMarket: any }) {
  ...
  return (
    <div className="rounded-3xl border border-border/50 bg-gradient-to-b from-card to-background p-6 space-y-5 shadow-sm backdrop-blur-xl">
      <div className="flex items-center justify-between">
        ...
        >
          {MARKET_SESSIONS[activeMarket].map(t => <option key={t} value={t}>Draw {MARKET_SESSION_LABELS[activeMarket][t]}</option>)}
        </select>
      </div>
...
export default function AnalisaHarianPage() {
  ...
  const { data: latest } = useGetTotoLatest(activeMarket);

  const analyses = useMemo(() => {
    if (!months) return [];
    return MARKET_SESSIONS[activeMarket].map((slot) => buildAnalysis(months, latest, slot as DrawTime));
  }, [months, latest]);

  // Overall today summary
  const todaySummary = useMemo(() => {
    if (!latest) return null;
    return MARKET_SESSIONS[activeMarket].map((slot) => {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-5 px-4 pb-24 pt-6 md:pb-6">
      <PageSeo title={`Analisa Harian - ${MARKET_NAMES[activeMarket]}`} description={`Analisa statistik per slot jam untuk strategi pasang 2D ${MARKET_NAMES[activeMarket]}`} />
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold text-foreground tracking-tight">Analisa Harian</h1>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2 bg-muted/20 p-1.5 rounded-2xl border border-border/50 shrink-0">
          ...
          <div className="flex flex-wrap gap-2">
            {todaySummary.map((s) => s && (
              <div key={s.slot} className="flex items-center gap-2 rounded-lg border border-border bg-card px-3 py-2">
                <span className="font-mono text-xs font-bold text-muted-foreground">{MARKET_SESSION_LABELS[activeMarket][s.slot]}</span>
      ...
      {/* Trend Chart */}
      {analyses.length > 0 && <TrendChart analyses={analyses} activeMarket={activeMarket} />}
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {analyses.map((a, i) => (
            <SlotCard key={a.slot} analysis={a} activeMarket={activeMarket} defaultOpen={i === 0} />
          ))}
        </div>
      )}
