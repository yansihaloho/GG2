import { useMarketStore, MARKET_SESSIONS, MARKET_SESSION_LABELS, MARKET_NAMES } from "@/lib/market-store";
...
// Remove DRAW_LABELS since we will use MARKET_SESSION_LABELS[activeMarket]
...
export default function RiwayatPrediksiPage() {
  ...
  const { data, isLoading, refetch } = useGetPredictions(activeMarket);
  const deleteMut = useDeletePrediction(activeMarket);
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-5 px-4 pb-24 pt-6 md:pb-6">
      <PageSeo title={`Riwayat Prediksi - ${MARKET_NAMES[activeMarket]}`} description={`Riwayat prediksi AI vs hasil aktual ${MARKET_NAMES[activeMarket]}`} />

      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <History className="h-6 w-6 text-primary" />
          <h1 className="text-2xl font-bold text-foreground font-mono tracking-tight">Riwayat Prediksi</h1>
          ...
          {sessions.length > 1 && (
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {sessions.map((s) => (
                ...
                >
                  {MARKET_SESSION_LABELS[activeMarket][s] ?? s} ({preds.filter(p => p.session === s).length})
                </button>
              ))}
            </div>
          )}
