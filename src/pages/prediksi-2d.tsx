...
export default function Prediksi2DPage() {
  ...
  const predictionList = useMemo<LineResult[]>(() => {
    ...
    const results: LineResult[] = Array.from({ length: 100 }, (_, i) => {
      ...
      // Weighting: Trend (30%) + All-Time Freq (10%) + Transition Trend (20%) + Position Strength (15%) + Overdue Gap Factor (10%) + Macro Trend (15%)
      const totalScore = (dScore * 0.3) + (fScore * 0.1) + (tScore * 0.2) + (cScore * 0.15) + (gapBonus * 0.1) + (trendBonus * 0.15);
      ...
      return {
        ...
        comboScore: Math.round(cScore), // We map comboScore to cScore still, could combine with trendBonus if we want
  ...
  const backtestStats = useMemo(() => {
    ...
    for (let i = 0; i < K; i++) {
      ...
      for (let i = histLength - tempRecentWindow; i < histLength; i++) {
          const full4d = priorHistory[i].result;
          const val2d = parseInt(full4d[2] + full4d[3]);
      ...
      priorHistory.forEach((h, idx) => {
        ...
        if (idx > 0) {
          const prevVal = priorHistory[idx - 1].result[2] + priorHistory[idx - 1].result[3];
          if (!tempTransitions[prevVal]) tempTransitions[prevVal] = new Array(100).fill(0);
      ...
      const tempScores = Array.from({ length: 100 }, (_, code) => {
        ...
        return { code, score: (dScore * 0.3) + (fScore * 0.1) + (tScore * 0.2) + (cScore * 0.15) + (tempTrend * 0.15) };
      });
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 px-4 pb-24 pt-6 md:pb-6">
      <PageSeo
        ...
        description={`Analisis multi-algoritma data historis ${MARKET_NAMES[activeMarket]} untuk menghasilkan 70 line 2D belakang paling akurat.`}
      />
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-3xl border border-border/50 bg-gradient-to-b from-card to-background p-6 space-y-5 shadow-sm backdrop-blur-xl">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 text-xs">
              ...
              <div className="space-y-1 bg-black/10 border border-border/20 rounded-xl p-3">
                <span className="font-bold text-emerald-400 block">4. Position & Rebound (25%)</span>
                <p className="text-[11px] text-muted-foreground leading-relaxed">
                  Menganalisis kekuatan digit Kepala/Ekor dan menyisipkan kompensasi angka overdue agar tidak tertinggal.
                </p>
              </div>
            </div>
          </div>
