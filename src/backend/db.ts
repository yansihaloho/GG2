...
export class Database {
  ...
  getTotoLatest(market = "toto"): TotoDay {
    let results = this.data.totoResults;
    if (market === "hk") results = this.data.hkResults || [];
    if (market === "sgp") results = this.data.sgpResults || [];
    if (market === "sdy") results = this.data.sdyResults || [];
    return results[0];
  }

  getTotoMonths(market = "toto"): TotoMonthGroup[] {
    let results = this.data.totoResults;
    if (market === "hk") results = this.data.hkResults || [];
    if (market === "sgp") results = this.data.sgpResults || [];
    if (market === "sdy") results = this.data.sdyResults || [];
  ...
  getPredictions(market: string = "toto"): Prediction[] {
    // Auto-update pending predictions
    this.updatePendingPredictions(market);
  ...
  async syncMarket(market: string, url: string) {
    console.log(`Scraping ${market} from ${url}`);
    try {
      ...
      const today = new Date();
      const anchor = new Date(today.getFullYear(), today.getMonth(), today.getDate(), 12, 0, 0);
      let idCounter = 1;
      
      $('.Tahunan-Tab').each((i, el) => {
        const dayName = $(el).find('th').text().trim().toUpperCase();
        const targetDayOfWeek = DAY_MAP[dayName];
        if (targetDayOfWeek === undefined) return;
        
        const col: string[] = [];
        $(el).find('td').each((j, td) => {
           col.push($(td).text().trim());
        });
        
        let dateObj = new Date(anchor.getTime());
        while (dateObj.getDay() !== targetDayOfWeek) {
          dateObj.setDate(dateObj.getDate() - 1);
        }
        
        for (let j = col.length - 1; j >= 0; j--) {
          const val = col[j];
          if (val && val !== '0000' && val.length === 4) {
            parsedResults.push({
              id: idCounter++,
              drawDate: dateObj.toISOString().slice(0, 10),
          ...
          if (dateObj.getFullYear() < 2026) break;
        }
      });
      
      parsedResults.sort((a, b) => b.drawDate.localeCompare(a.drawDate));
      
      // Re-assign IDs based on sorted order so latest has highest ID if desired (or just leave them)
      
      if (market === 'hk') this.data.hkResults = parsedResults;
      if (market === 'sgp') this.data.sgpResults = parsedResults;
      if (market === 'sdy') this.data.sdyResults = parsedResults;
      this.save();
    } catch(err) {
  ...
  async syncWithLiveWebsite(): Promise<number> {
    ...
    this.syncMarket("hk", "https://masterlive.net/data-hk-lengkap-2026.php"); this.syncMarket("sgp", "https://masterlive.net/data-sgp-lengkap-2026.php"); this.syncMarket("sdy", "https://masterlive.net/data-sdy-lengkap-2026.php"); console.log("Scraping live 2026 Toto Macau data from masterlive.net...");
    try {
  ...
  private generatePredictionsForToday() {
    ...
    const markets = ["toto", "hk", "sgp", "sdy"] as const;
    const marketSessions: Record<string, string[]> = {
      toto: ["0001", "1300", "1600", "1900", "2200", "2300"],
      hk: ["0001"],
      sgp: ["0001"],
      sdy: ["0001"]
    };
    ...
    for (const market of markets) {
      ...
      const months = this.getTotoMonths(market);

      for (const session of sessions) {
        const exists = this.data.predictions.some((p) => p.market === market && p.forDate === todayStr && p.session === session);
        if (!exists) {
          try {
            const predResult = runPrediction(months as any, session as any);
            if (predResult) {
            ...
            console.error(`Failed to generate AI prediction for ${market} session ${session}:`, err);
          }
        }
      }
    }
  ...
  private updatePendingPredictions(marketFilter?: string) {
    let changed = false;
    ...
    for (const pred of this.data.predictions) {
      if (marketFilter && pred.market !== marketFilter) continue;
      ...
      if (pred.actual4d === null) {
        ...
        let results = this.data.totoResults;
        if (pred.market === "hk") results = this.data.hkResults || [];
        if (pred.market === "sgp") results = this.data.sgpResults || [];
        if (pred.market === "sdy") results = this.data.sdyResults || [];

        const result = results.find((r) => r.drawDate === pred.forDate);
        if (!result) continue;
